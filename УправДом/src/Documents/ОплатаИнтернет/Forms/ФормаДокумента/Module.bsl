...
Процедура ПриОткрытии(Отказ)
	...
	УчитыватьДопУслуги = Объект.УчитыватьДопУслугиИнтернет;
	Элементы.ДанныеПлатежаТарифДопУслугиИнтернет.Видимость = УчитыватьДопУслуги;
...
&НаКлиенте
Процедура УчитыватьДопУслугиИнтернетПриИзменении(Элемент)
...
Процедура УстановитьДоступностьЭлементов()
	...
	Элементы.УчитыватьДопУслугиИнтернет.ТолькоПросмотр = Объект.Проведен;
...
Процедура СозданиеГруппПрисоединенныхФайлов()
	...
	ИмяСправочника = "ОплатаИнтернетПрисоединенныеФайлы"; // Имя справочника для этого документа
...
Процедура ЗаполнитьДанныеПлатежаНаСервере()
	...
	Если Объект.УчитыватьДопУслугиИнтернет = Истина Тогда							 
		ТарифДопУслугиИнтернет = РегистрыСведений.ТарифыКоммунальныхУслуг.ПолучитьАктуальныйТариф(
		  Объект.ДопУслугиИнтернетСлужебный, Объект.ОплачиваемыйПериодСлужебный);
	
		// 2. Проверяем, все ли данные в наличии для заполнения
		Если ТарифДопУслугиИнтернет = 0 Тогда
			СообщениеОКоличестве = Новый СообщениеПользователю;
			СообщениеОКоличестве.Текст = "Не найден актуальный тариф допуслуг для расчета.";
			СообщениеОКоличестве.Поле = "Объект.ОплачиваемыйМесяц"; 
			СообщениеОКоличестве.Сообщить();
			Возврат;
		КонецЕсли;
	Иначе 
		ТарифДопУслугиИнтернет = 0;
	КонецЕсли;
	...
	СуммаПлатежа = ТекущийТариф + ТарифДопУслугиИнтернет;
	...
	НоваяСтрока.ТарифИнтернет = ТекущийТариф;
	НоваяСтрока.ТарифДопУслугиИнтернет = ТарифДопУслугиИнтернет;
